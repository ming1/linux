 ...
 retry:
	...
	if ((r->res_master_nodeid != our_nodeid) && from_other) {
		/* our rsb is not master, and another node (not the dir node)
		   has sent us a request.  this is much more common when
		   our master_nodeid is zero, so limit debug to non-zero. */
		if (r->res_master_nodeid) {
			log_debug(ls, "find_rsb keep from_other %d master %d "
