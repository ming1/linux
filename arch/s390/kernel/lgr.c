...
#define ZVM_LEVEL_MAX 2 /* Maximum is 8, but we only record two levels */
...
static void lgr_stsi_3_2_2(struct lgr_info *lgr_info)
{
	...
	if (stsi(si, 3, 2, 2) == -ENOSYS)
		return;
	for (i = 0; i < min_t(u8, si->count, ZVM_LEVEL_MAX); i++) {
...
static void lgr_info_get(struct lgr_info *lgr_info)
{
	...
	stfle(lgr_info->stfle_fac_list, sizeof(lgr_info->stfle_fac_list));
...
{
	static DEFINE_RAW_SPINLOCK(lgr_info_lock);
	unsigned long flags;

	if (!lgr_page)
		return;
	if (!raw_spin_trylock_irqsave(&lgr_info_lock, flags))
		return;
	...
	raw_spin_unlock_irqrestore(&lgr_info_lock, flags);
}
...
static void lgr_timer_set(void)
{
	lgr_timer.function = lgr_timer_fn;
	lgr_timer.data = 0;
	lgr_timer.expires = jiffies + LGR_TIMER_INTERVAL_SECS * HZ;
	add_timer(&lgr_timer);
}
...
static int lgr_init(void)
{
	...
	init_timer_deferrable(&lgr_timer);
	lgr_timer_set();
	return 0;
}
