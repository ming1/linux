...
#endif
	if (pcpu != &pcpu_devices[0]) {
		free_page(pcpu->panic_stack - PAGE_SIZE);
		free_pages(pcpu->async_stack - ASYNC_SIZE, ASYNC_ORDER);
		free_pages((unsigned long) pcpu->lowcore, LC_ORDER);
	}
}
 ...
 */
static void __noreturn pcpu_delegate(struct pcpu *pcpu, void (*func)(void *),
 ...
 */
void __noreturn smp_call_online_cpu(void (*func)(void *), void *data)
{
	struct pcpu *pcpu;
...
{
	pcpu_delegate(pcpu_devices, func, data, pcpu_devices->panic_stack);
}
...
void __init smp_prepare_boot_cpu(void)
{
	...
	pcpu->async_stack = S390_lowcore.async_stack;
	pcpu->panic_stack = S390_lowcore.panic_stack;
