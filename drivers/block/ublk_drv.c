...
struct ublk_queue {
	...
	struct {
		...
		/* batch fetch command list */
		struct list_head fcmd_head;

		/* anytime there is at most one active fetch command */
...
ublk_batch_alloc_fcmd(struct io_uring_cmd *cmd)
{
	...
	unsigned buf_group = READ_ONCE(cmd->sqe->buf_index);

	if (fcmd) {
		fcmd->cmd = cmd;
		fcmd->buf_group = buf_group;
	}
	return fcmd;
}
			      ...
			      unsigned int issue_flags)
{
	...
	data->buf_ctx_handle = io_uring_cmd_ctx_handle(cmd);
	/* store buffer index in request payload */
	data->buf_index = io->buf.index;
				   ...
				   unsigned int issue_flags)
{
	if (ublk_support_auto_buf_reg(ubq) && ublk_rq_has_data(req))
		return ublk_auto_buf_reg(req, io, cmd, issue_flags);
				       ...
				       unsigned short tag)
{
	...
	if (!ublk_prep_auto_buf_reg(ubq, req, io, data->cmd,
				data->issue_flags))
		return false;

	spin_lock(&io->lock);
	ublk_prep_compl_io_cmd(io, req, UBLK_IO_RES_OK);
	spin_unlock(&io->lock);
...
static inline void __ublk_queue_rqs(struct rq_list *rqlist, bool batch)
{
...
static inline int ublk_check_auto_buf_reg(const struct ublk_auto_buf_reg *buf)
{
...
static int ublk_init_auto_buf_reg(struct ublk_queue *ubq,
				  struct io_uring_cmd *cmd,
				  struct ublk_auto_buf_reg *buf)
{
	...
	*buf = ublk_sqe_addr_to_auto_buf_reg(READ_ONCE(cmd->sqe->addr));
	return ublk_check_auto_buf_reg(buf);
}
...
static int __ublk_fetch(struct io_uring_cmd *cmd, struct ublk_queue *ubq,
			struct ublk_io *io, struct ublk_cmd_data *data)
{
	struct ublk_device *ub = ubq->dev;
	int ret = 0;
		      ...
		      struct ublk_io *io, struct ublk_cmd_data *data)
{
	 ...
	 */
	mutex_lock(&ub->mutex);
	ret = __ublk_fetch(cmd, ubq, io, data);
	mutex_unlock(&ub->mutex);
...
static void ublk_commit_and_fetch(const struct ublk_queue *ubq,
				  ...
				  unsigned int issue_flags,
				  const struct ublk_cmd_data *cmd_data)
{
	if (ublk_support_auto_buf_reg(ubq)) {
	...
	if (req_op(req) == REQ_OP_ZONE_APPEND)
		req->__sector = cmd_data->zone_append_lba;
...
__ublk_get_data(struct ublk_io *io, const struct ublk_cmd_data *data)
{
	struct request *req = io->req;
			       ...
			       const struct ublksrv_io_cmd *ub_cmd)
{
	...
	struct ublk_cmd_data data = {
		...
		.addr = ub_cmd->addr,
		.zone_append_lba = ub_cmd->zone_append_lba,
	};
	...
	case UBLK_IO_FETCH_REQ:
		...
		if (ret)
			goto out;
		ret = ublk_init_auto_buf_reg(ubq, cmd, &data.auto_buf);
		if (ret)
			goto out;
		ret = ublk_fetch(cmd, ubq, io, &data);
		if (ret)
			goto out;
		break;
	case UBLK_IO_COMMIT_AND_FETCH_REQ:
		...
		if (ret)
			goto out;
		req = ublk_fill_io_cmd(io, cmd, &data);
		if (!req) {
			ret = -EINVAL;
			goto out;
		}
		ublk_commit_and_fetch(ubq, cmd, req, issue_flags, &data);
		break;
	case UBLK_IO_NEED_GET_DATA:
		return ublk_get_data(ubq, io, &data);
	default:
		goto out;
	}
					...
					const struct ublk_elem_header *elem)
{
	...
	if (uc->flags & UBLK_BATCH_F_HAS_BUF_ADDR)
		return *(__u64 *)(buf + uc->elem_bytes - 8);
	return -1;
}

static inline __u16 ublk_batch_buf_idx(const struct ublk_batch_io *uc,
				       const struct ublk_elem_header *elem)
{
	const void *buf = (const void *)elem;

	if (uc->flags & UBLK_BATCH_F_HAS_BUF_INDEX)
		return *(__u16 *)(buf + uc->elem_bytes - 8);
	return -1;
}
...
ublk_batch_auto_buf_reg(const struct ublk_batch_io *uc,
			const struct ublk_elem_header *elem)
{
	__u16 idx = ublk_batch_buf_idx(uc, elem);
...
{
	unsigned int nr_queues = data->dev->dev_info.nr_hw_queues;
	int i, ret = 0;
	...
	for (i = 0; i < bytes; i += iter->elem_bytes) {
		...
		if (unlikely(elem->q_id >= nr_queues)) {
			ret = -EINVAL;
			break;
		}

		ubq = ublk_get_queue(data->dev, elem->q_id);
		if (unlikely(elem->tag >= ubq->q_depth)) {
			ret = -EINVAL;
			break;
		}
...
static int ublk_validate_batch_fetch_cmd(struct ublk_batch_io_data *data)
{
	...
	if (uc->q_id >= nr_queues)
		return -EINVAL;

	data->ubq = ublk_get_queue(data->dev, uc->q_id);
	return 0;
}
