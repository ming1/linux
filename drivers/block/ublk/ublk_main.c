...
static inline struct io_uring_cmd *__ublk_prep_compl_io_cmd(struct ublk_io *io,
							    struct request *req)
{
...
static void ublk_queue_rqs(struct rq_list *rqlist)
{
	...
	while ((req = rq_list_pop(rqlist))) {
		...
		if (io && (io_uring_cmd_ctx_handle(io->cmd) !=
			   io_uring_cmd_ctx_handle(this_io->cmd) ||
			   io->task != this_io->task) &&
...
int ublk_register_io_buf(struct io_uring_cmd *cmd,
			struct ublk_device *ub,
			u16 q_id, u16 tag,
			struct ublk_io *io,
...
static void __ublk_deinit_queue(struct ublk_device *ub, struct ublk_queue *ubq)
{
	...
	if (ublk_dev_support_batch_io(ub)) {
		WARN_ON_ONCE(!kfifo_is_empty(&ubq->evts_fifo));
		kfifo_free(&ubq->evts_fifo);
	}
...
static int ublk_init_queue(struct ublk_device *ub, int q_id)
{
	...
	if (ublk_dev_support_batch_io(ub)) {
		spin_lock_init(&ubq->evts_lock);
		ret = kfifo_alloc_node(&ubq->evts_fifo, ubq->q_depth,
